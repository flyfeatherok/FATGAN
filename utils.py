...
class ImageData:
    ...
    def __init__(self, data_path, img_shape=(64,64,1), augment_flag=False, data_type='None', img_type='jpg', pad_flag=False, label_size=8):
        ...
        if self.data_type == 'CelebA':
            ...
            self.train_dataset, self.train_label, self.random_label = self.CelebaA.CelebaA_Data_Label()
#            print(self.random_label)
    ...
    def image_processing(self, filename, label, random_label):
        ...
        return img, label, random_label

class CelebaA_Data:

    def __init__(self, data_path, label_size) :
        self.data_path = data_path
        self.lines = open(os.path.join(data_path, 'Anno/list_attr_celeba.txt'), 'r').readlines()
        self.label_size = label_size
        self.labels = ['Male','Pale_Skin',
                       'Black_Hair','Blond_Hair','Brown_Hair',
                        'Eyeglasses','Smiling','Young',
                        'Mouth_Slightly_Open','Bangs','Bald','Gray_Hair']
        self.phase_size = self.label_size-3
        self.Label_Mode={}
    ...
    def CelebaA_Data_Label(self):
        ...
        for line in self.lines:
            label=[0.0]*self.label_size
            split = line.split()
            filename = os.path.join(self.data_path, 'celeba_128',split[0])
            att_value = split[1:]
            ...
            labels.append(label)
            Random_Labels.append(self.CelebaA_Random_Label(label))
        return data, labels, Random_Labels
    
    def CelebaA_Random_Label(self, label):
        Random_label=[0.0]*self.label_size
        for i in range(self.label_size):
            p = random.random()
            if self.labels[i].__contains__('Hair'):
                continue
            else:
                if p<0.5:
#                    label[self.Label_Mode[self.labels[i]]] = 1
                    Random_label[i] = 1-label[i]
        p = random.random()
        if p < 0.33:
            Random_label[self.Label_Mode['Black_Hair']]=1
        elif p<0.66:
            Random_label[self.Label_Mode['Blond_Hair']]=1
        elif p<1:
            Random_label[self.Label_Mode['Brown_Hair']]=1
        # elif p<1:
        #     Random_label[self.Label_Mode['Gray_Hair']]=1
        return Random_label
